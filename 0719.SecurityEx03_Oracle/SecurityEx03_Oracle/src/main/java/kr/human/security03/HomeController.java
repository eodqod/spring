...
public class HomeController {
	...
	*/
	...
	// 암호화를 위하여 객체를 자동 주입받는다.
	@Autowired
	private BCryptPasswordEncoder bCryptPasswordEncoder;
	
	// 평문을 암호화 해서 출력해보기
	@RequestMapping(value = "/getPassword")
	public String getPassword(@RequestParam(value = "pwd", defaultValue = "admin", required = false)String password,
			Model model) {
	...
	public String passwordCheck(@RequestParam(value = "pwd", defaultValue = "admin", required = false)String password,
			Model model) {
		String newPassword = bCryptPasswordEncoder.encode(password);
		String dbPassword = bCryptPasswordEncoder.encode(password);
		boolean isCheck1 = newPassword.equals(dbPassword);
		boolean isCheck2 = bCryptPasswordEncoder.matches(password, dbPassword); // 이렇게 비교해야 된다.
